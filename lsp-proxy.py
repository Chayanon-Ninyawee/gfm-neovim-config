...
def main():
    ...
    if len(sys.argv) < 5:
        logger.error(
            "Usage: proxy.py <user@remote> <sshfs_prefix> <remote_prefix> [--root-dir <dir>] <lsp_command> [args...]"
        )
        sys.exit(1)
    ...
    try:
        ...
        if needs_env_setup:
            ...
            logger.info(
                f"Using direct command for LSP server: {cd_command}{lsp_command_str}"
            )
